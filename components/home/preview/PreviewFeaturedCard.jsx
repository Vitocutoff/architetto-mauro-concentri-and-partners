...
export default function PreviewFeaturedCard({
  featured,
  fontSans,
  fontSerif,
  ...
  // Se non passi reduceMotion dal parent, lo ricaviamo qui
  const reduceMotionHook = useReducedMotion();
  const reduceMotion =
    typeof reduceMotionProp === "boolean" ? reduceMotionProp : reduceMotionHook;

  // ✅ Motion SOLO dopo hydration e solo se non reduce motion
      ...
      <a href={featured.link} className="block">
        <div className="relative h-72 sm:h-80 lg:h-112">
          ...
          <div className="absolute bottom-5 left-5 right-5">
            <div className={`${fontSerif} text-2xl sm:text-3xl text-white/95`}>
              {featured.title}
            </div>

            <div className={`${fontSans} mt-1 text-sm uppercase text-right text-white/70`}>
              Scopri →
            </div>
          </div>
        </div>
